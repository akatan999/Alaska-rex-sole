---
title: "Diagnostics and Sensitivity of Stock Synthesis model for Alaska rex sole"
...
This script is now tailored on ensemble grid of several model configurations with several survey, length and age data slot. It can be modified to include more survey, length or age compositions.       
Age slot have to be activate in the Diagnostic loop before running the script (now only Length active)  
Mohn Rho value bound set now for long-live species, please change for short-live species           In Ensemble part make sure you set correct Fref depending on your choice (es. MSY, Btgt,..)                         
Weighting vector automatically produce is a pure mean of all diagnostic scores as they are,       if the user want to use different Weighting methods (i.e., merge multiple similar diagnostics into a single score) have to produce the vector externally and save it in the main dir as "weight_vector.csv"                           

## Load packages and setting the scene
...
## This is an ICES centric set up to define limit and targets
RATIOLIM  = 2.667 # ratio of Btarget on Blim: e.g. if target=SSB35% and lim=SSB15% --> 40/15=2.667
...
CTL_FILE      = "GOA_Rex_8_2021.ctl"
DAT_FILE      = "GOA_Rex_8_2021.dat"

##You can choose between number or names for identifying the different models (example here with  model names and two models of the grid)
#runs <- c("Reference_run", paste0("Run", c(1:6))) # 1:N Number of run     
runs <- c("basecase", "run9_early_maturity", "run10_late_maturity", "run11_est_male_M", "basecase_TVsel_fleet", "basecase_TVM", "basecase_TVsel_fleet_TVM", "basecase_TVsel_fleet_TVM_h072")  
nmodels <- 8
...
if (Sys.info()["sysname"] == "Windows") {
    ...
    SS_EXE   <- "~/Max/Stock_synthesis/ss3_3.23.1/ss3.exe"
...
print("Model runs done above, now generating SS standard plots, one at a time."); flush.console()
Sys.sleep(3)

for (i in 1:length(runs)){
    tmp <- SS_output(dir = file.path(runs[i]))
    SS_plots(tmp, uncertainty=T, datplot=F, forecast=T, maxyr=2030, minbthresh=0.25,fitrange = F)
}
```


# Build the empty diags table for weighting the models later (more slots can be added for more complex models)
...
## Make files for retro & change starter file. Needed to allow retro analysis to run on multiple cores but it should be done before to run the markdown and ideally on a cluster
    ...
    for(j in 1:length(retro_files)){
        ...
        file.copy(paste(dir.runN, "starter.ss", sep="/"),
                  paste(dir.runN.new, "starter.ss", sep="/"), overwrite=TRUE)
                                        ...
                                        # copy other files
        file.copy(paste(dir.runN, "GOA_Rex_8_2021.ctl", sep="/"),
                  paste(dir.runN.new, "GOA_Rex_8_2021.ctl", sep="/"), overwrite=TRUE)
        file.copy(paste(dir.runN, "GOA_Rex_8_2021.dat", sep="/"),
                  paste(dir.runN.new, "GOA_Rex_8_2021.dat", sep="/"), overwrite=TRUE)
...
for(i in runs){
  ...
  results$N_Params[results$Run== i] <-   tmp$N_estimated_parameters
    ...
    sspar(mfrow=c(2,2),plot.cex = 0.8)   # change to 3 if also Age present
    ...
    results$RMSE_Perc_2[results$Run== i] <-   jr.age$RMSE.perc[jr.age$indices=="Combined"]

    ## Check Runs Test
    sspar(mfrow=c(2,3),plot.cex = 0.8)    # change based on number of fleet and length/age data
...
##############################
    ...
    save(ss3rep, file=paste0(dir = rdata,"/Alaska rex sole_",i,".Rdata")) 
  ...
  ##############################
    ...
    sspar(mfrow=c(1,2),plot.cex = 0.9)     # change based on number of cpue or survey
    ...
    #mase_len.plot2 <- SSplotHCxval(hccomps,add=T,subplots = "len",legendloc="topright",legend = FALSE, indexUncertainty = T,legendcex = 1)
    ...
    dev.print(jpeg,paste0(dir = dir.diag,"/HCxval_len&age_",i,".jpg"), width = 8, height = 9, res = 300, units = "in")
...
results2$RMSE_Perc_2 <- ifelse(results2$RMSE_Perc_1 < 30, 1, 0)
...
for(i in 1:length(runs)){
    ...
    mvn.temp = SSdeltaMVLN(run, run = runs[i],
                           ...
                           addprj = TRUE, mc = MVLN_MC, weight = weight_vector[i], Fref = "Btgt", plot=FALSE, verbose=TRUE, catch.type = c("Exp"))
    kbproj = rbind(kbproj,
...
## Kobe_plot ensemble
```{r fit1, fig.height=8,fig.width=10, fig.cap = paste0("Kobe_plot ensemble points")}
...
```{r fit2, fig.height=8,fig.width=10, fig.cap = paste0("Kobe_plot ensemble kernel")}
sspar(mfrow=c(1,1),plot.cex = 0.7)
...
```{r fit3, fig.height=8,fig.width=10, fig.cap = paste0("Trajectories")}
...
for (i in 1:2) {
    ...
    SSplotEnsemble(kbproj, add=T, subplots = quants[1], legendcex = 0.6, legendloc="bottomright", legendncol = 3, ylabs = labels[1], xlim = years, lwd=1)
    abline(h = 1/RATIOLIM, lty = 1)   # Add SSB/Blim horizontal line
    SSplotEnsemble(kbproj, add=T, subplots = quants[2:6], legendcex = 0.6, legendloc="bottomright", legendncol = 5, ylabs = labels[2:6], xlim = years, lwd=1)
